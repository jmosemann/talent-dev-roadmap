...
export default function App() {
  ...
  async function generateReport() {
    ...
    try {
      ...
      if (!startResponse.ok) throw new Error(start.error || `Report generation could not start (${startResponse.status}).`)
      if (!start.responseId) throw new Error('The report job started without a response ID. Please try again.')

      const result = await pollReport(start.responseId)
  ...
  return <div className="app-shell">
    ...
    <section className="hero no-print"><div className="hero-copy"><Eyebrow>Herohub Report Studio</Eyebrow><h1>Turn a Talent Development Roadmap into a polished Herohub report.</h1><p>Upload the standard Herohub Talent Development Roadmap PDF. The app now maps its recurring sections—including individual development calendars—into a consistent branded report.</p></div>
      <div className="upload-card"><input ref={inputRef} type="file" accept=".md,.txt,.pdf,text/markdown,text/plain,application/pdf" onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedFile(e.target.files?.[0] || null)} hidden />
        ...
        <button className="primary-button" onClick={generateReport} disabled={!selectedFile || isGenerating}>{isGenerating ? 'Analyzing roadmap… this can take a few minutes' : 'Generate Report'}</button><div className="or"><span />or<span /></div><button className="secondary-button" onClick={showSample}>Preview Included Sample</button>{error && <div className="error-box">{error}</div>}
      </div></section>
    ...
    <footer className="app-footer no-print"><Logo /><span>Herohub Roadmap Builder · v1.1.2</span></footer>
  </div>
}


async function pollReport(responseId: string): Promise<{ report: HerohubReport }> {
  const deadline = Date.now() + 10 * 60 * 1000
  while (Date.now() < deadline) {
    await sleep(2500)
    const response = await fetch(`/api/generate-report?responseId=${encodeURIComponent(responseId)}`, {
      method: 'GET',
      headers: { 'Accept': 'application/json' },
      cache: 'no-store'
    })
    const result = await readApiResponse(response)
    if (!response.ok) throw new Error(result.error || `Report generation status check failed (${response.status}).`)
    if (result.status === 'completed' && result.report) return { report: result.report as HerohubReport }
    if (['failed', 'cancelled', 'incomplete'].includes(result.status)) {
      throw new Error(result.error || `OpenAI ended the report job with status “${result.status}”.`)
    }
  }
  throw new Error('The report is still processing after 10 minutes. Please try again or check the Netlify function logs.')
}
...
async function readApiResponse(response: Response): Promise<Record<string, any>> {
  ...
  try {
    ...
    const looksHtml = contentType.includes('text/html') || /^\s*<!?html/i.test(raw) || /^\s*</.test(raw)
    if (looksHtml) {
      const detail = response.status >= 500
        ? 'The Netlify server function returned an HTML error page. This is usually a function timeout or runtime error.'
        : 'The report API route returned the website HTML instead of JSON. Confirm that the Netlify function deployed with this version.'
      throw new Error(`${detail} HTTP ${response.status}.`)
    }
    throw new Error(`The report API returned an unreadable response (HTTP ${response.status}).`)
  }
}
